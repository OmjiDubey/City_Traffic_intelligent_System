...
export function KpiCard({
  ...
  return <div className="flex min-w-0 items-start gap-4 rounded-lg border border-line bg-surface p-4 shadow-card">
      <div className={twMerge("flex h-11 w-11 shrink-0 items-center justify-center rounded-md", tileClasses[tone])}>
        <Icon className="h-5 w-5" aria-hidden="true" />
      </div>
      <div className="min-w-0">
        <p className="truncate text-sm text-muted">{label}</p>
        <p className="tabular mt-0.5 truncate text-[1.625rem] font-bold leading-tight text-navy">{value}</p>
